...
class united_model(torch.nn.Module):
    ...
    def forward(self, x):
        feat_lr = self.encoder_lr.get_intermediate_layers(x, n=4, reshape=True, norm=False)[0]
...
class MemIRv1Model(BaseModel):
    def __init__(self, opt):
        super(MemIRv1Model, self).__init__(opt)
    ...
    def setup_optimizers(self):
        train_opt = self.opt['train']
        optim_params = self.net_g.decoder.parameters() #[]
