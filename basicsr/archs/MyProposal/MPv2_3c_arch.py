'''
MPv2_3c: Per-Layer LoRA Adaptation (P4 ablation)

Based on MPv2_3_arch, moving alpha_proj from BasicBlock to each ATDTransformerLayer.

=== Improvement over MPv2_3 ===

Problem (P4): In MPv2_3, TD is adapted once at BasicBlock entry, then all 6 layers
share the same adapted TD. Each layer processes different abstraction levels but
receives identical memory patterns.

Solution: Each ATDTransformerLayer has its own alpha_proj, generating layer-specific
TD adaptation. The static base TD (self.td) and LoRA matrices (lora_A, lora_B) remain
shared in BasicBlock, but each layer independently decides which memory patterns to activate.

Key difference from MPv2_3:
- alpha_proj moved from BasicBlock to each ATDTransformerLayer
- BasicBlock.forward: constructs td_base once, passes to each layer
- Each layer generates its own alpha and adapts td_base independently
- Extra params: ~1.5K/layer x 24 layers = ~37K total
...
class ATDTransformerLayer(nn.Module):
    """ATD layer with per-layer LoRA alpha generation + additive timestep injection."""
    ...
    def forward(self, x, td_base, x_size, params, temb=None, lora_A=None, lora_B=None, x_lq=None):
        h, w = x_size
        b, n, c = x.shape
        c3 = 3 * c

        # ===== Per-layer TD adaptation =====
        if x_lq is not None and lora_A is not None:
            ...
            td_lowrank = td_base @ lora_A.T  # [B, M, r]
            td_lowrank = td_lowrank * alpha.unsqueeze(1)
            delta_td = td_lowrank @ lora_B.T
            td = td_base + delta_td
        else:
            td = td_base
...
class BasicBlock(nn.Module):
    """BasicBlock with per-layer LoRA adaptation.

    TD base and LoRA matrices are shared, but each layer generates its own alpha.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
        ...
        # LoRA decomposition (shared across layers)
    ...
    def forward(self, x, x_size, params, temb=None, x_lq=None):
        b, n, c = x.shape
        td_base = self.td.unsqueeze(0).expand(b, -1, -1)  # [B, M, C]

        # Pass td_base, lora_A, lora_B, x_lq to each layer for per-layer adaptation
        for layer in self.layers:
            x = layer(x, td_base, x_size, params, temb,
                      lora_A=self.lora_A, lora_B=self.lora_B, x_lq=x_lq)
...
class MPv2_3c_arch(nn.Module):
    """MPv2_3c: Per-Layer LoRA Adaptation + LoRA-TD.

    Key change from MPv2_3: Each ATDTransformerLayer has its own alpha_proj,
    generating layer-specific TD adaptation.
    """
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # Deep Feature Extraction (Per-Layer LoRA-TD)
