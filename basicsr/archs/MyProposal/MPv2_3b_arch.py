'''
MPv2_3b: Spatial-Aware LoRA Conditioning (P3 ablation)

Based on MPv2_3_arch, replacing global-average-pooled lq with spatial-aware alpha generation.

=== Improvement over MPv2_3 ===

Problem (P3): In MPv2_3, lq_global = x_lq.mean(dim=1) discards all spatial structure.
The same alpha [B, r] activates memory patterns identically for every spatial location,
losing per-region selectivity.

Solution: Generate alpha per-token from x_lq, then aggregate spatially:
1. x_lq_t = x_lq + temb_broadcast  (inject timestep into each lq token)
2. alpha_spatial = sigmoid(alpha_proj(x_lq_t))  -> [B, N, r] per-token activations
3. alpha = alpha_spatial.mean(dim=1)  -> [B, r] spatially aggregated

This preserves spatial information: different image regions contribute differently
to memory pattern selection. The aggregation ensures alpha is still [B, r] for TD.

Key difference from MPv2_3:
- BasicBlock.forward: alpha_proj takes x_lq tokens directly instead of GAP(lq_global)
- alpha_proj input dim = dim (not dim+temb_ch), temb added directly to x_lq tokens
  (valid because temb_ch == dim in our config)
- Parameters slightly reduced: alpha_proj input 96 vs 192

LoRA-TD decomposition and ATDTransformerLayer unchanged from MPv2_3.
...
class BasicBlock(nn.Module):
    """BasicBlock with Spatial-Aware LoRA-Conditioned Token Dictionary.

    Innovation: Generate alpha per-token from x_lq (not GAP), then aggregate spatially.
    This preserves spatial information in the condition signal.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
        ...
        # ===== Spatial-aware alpha projection =====
        # Input: x_lq tokens [B, N, dim] (temb added directly to tokens when temb_ch==dim)
    ...
    def forward(self, x, x_size, params, temb=None, x_lq=None):
        ...
        # ===== Spatial-aware LoRA memory extraction =====
        if x_lq is not None:
            # Inject timestep into each lq token
            if temb is not None and self.temb_ch > 0:
                x_lq_t = x_lq + temb.unsqueeze(1)  # [B, N, C]
            else:
                x_lq_t = x_lq

            # Per-token alpha generation, then spatial aggregation
            alpha_spatial = torch.sigmoid(self.alpha_proj(x_lq_t))  # [B, N, r]
            alpha = alpha_spatial.mean(dim=1)  # [B, r]

            # LoRA decomposition (same as MPv2_3)
...
class MPv2_3b_arch(nn.Module):
    """MPv2_3b: Spatial-Aware LoRA Conditioning + LoRA-TD.

    Key change from MPv2_3: Per-token alpha from x_lq instead of GAP, preserving spatial info.
    """
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # Deep Feature Extraction (Spatial-LoRA-TD)
