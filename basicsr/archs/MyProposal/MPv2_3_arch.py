'''
MPv2_3: LoRA-Conditioned Token Dictionary (LoRA-TD)
Based on MPv2_2, replacing the unused FiLM modulation with LoRA-based memory extraction.

=== Improvement Summary ===

Background:
- MPv2 (timestep-conditioned TD via AdaLN) FAILED because JiTModel's target x_1 is constant,
  making timestep-only conditioning meaningless for TD.
- MPv2_2 defined FiLM modulation (lq_td_proj) but never wired it into forward — TD remained static.
- Static TD provides the same 64 memory tokens for every image, regardless of content.

Key Innovation — LoRA-Conditioned Memory Extraction:
- Decompose static TD into low-rank "memory patterns" via learnable A [r, C] and B [C, r].
- Condition signal (lq_global + temb) generates per-rank activation weights alpha [B, r].
- alpha controls how much of each memory pattern to activate for the current image/timestep.
- This selectively extracts relevant memories from TD, making it content-aware and timestep-aware.

Pipeline (per BasicBlock):
    td:        [M, C]              static token dictionary (learnable)
    lq_global: [B, C]              image content descriptor (GAP of x_lq)
    temb:      [B, temb_ch]        timestep embedding

    Step 1: td_lowrank = td @ A^T              → [B, M, r]   project to r-dim pattern space
    Step 2: alpha = sigmoid(MLP(lq_global, t))  → [B, r]      per-pattern activation
    Step 3: td_selected = td_lowrank * alpha    → [B, M, r]   condition-weighted selection
    Step 4: delta_td = td_selected @ B^T        → [B, M, C]   project back to feature space
    Step 5: td_adapted = td + delta_td          → [B, M, C]   residual (static + adaptive)

Why LoRA (vs FiLM / AdaLN-Zero):
- FiLM: global scale+shift, no per-token selectivity. ~32K params/block.
- AdaLN-Zero: modulates LayerNorm + gates, expressive but heavy. ~48K params/layer.
- LoRA: rank-r decomposition, per-token selectivity via pattern activation. ~2.8K params/block.
- Zero-init B ensures td_adapted = td at start (same as static ATD, stable training).

Design:
- x_t: main processing path (shallow conv → transformer → output)
- lq: provides content signal for LoRA TD extraction (GAP → lq_global)
- t: provides timestep signal for LoRA TD extraction (joint condition with lq_global)
- lq also additively merged into main features at each ATDB stage (same as MPv2_2)

Forward signature: (x, c=None, temp=None) for JiTModel compatibility.
x = cat([x_t, lq_bicubic], dim=1) as 6ch, split internally.
'''
...
class ATDTransformerLayer(nn.Module):
    """ATD layer with simple additive timestep injection."""
...
class BasicBlock(nn.Module):
    """BasicBlock with LoRA-Conditioned Token Dictionary.

    Innovation: (lq_global, temb) → LoRA low-rank activation → adaptive TD.
    Static TD is decomposed into r memory patterns via A [r,C] and B [C,r].
    Condition (lq content + timestep) controls per-pattern activation alpha [B,r],
    selectively extracting relevant memories for the current image/timestep.
    Zero-init B ensures td_adapted = td at training start.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
        ...
        # Token Dictionary (static base)
        self.td = nn.Parameter(torch.randn([num_tokens, dim]), requires_grad=True)

        # ===== LoRA-Conditioned TD =====
        self.lora_rank = lora_rank

        # LoRA decomposition: A projects TD tokens into r-dim pattern space
        self.lora_A = nn.Parameter(torch.randn(lora_rank, dim) * 0.01)   # [r, C]
        # B projects back to feature space; zero-init for stable start
        self.lora_B = nn.Parameter(torch.zeros(dim, lora_rank))          # [C, r]

        # Condition → per-rank activation: (lq_global + temb) → alpha [B, r]
    ...
    def forward(self, x, x_size, params, temb=None, x_lq=None):
        b, n, c = x.shape
        td = self.td.unsqueeze(0).expand(b, -1, -1)  # [B, M, C]

        # ===== LoRA memory extraction =====
        if x_lq is not None:
            lq_global = x_lq.mean(dim=1)  # [B, C]

            # Build joint condition
            if temb is not None:
                cond = torch.cat([lq_global, temb], dim=-1)  # [B, C + temb_ch]
            else:
                cond = lq_global

            # Condition → per-rank activation
            alpha = torch.sigmoid(self.alpha_proj(cond))  # [B, r]

            # Step 1: project TD tokens into low-rank pattern space
            td_lowrank = td @ self.lora_A.T  # [B, M, r]

            # Step 2: condition-weighted selection
            td_lowrank = td_lowrank * alpha.unsqueeze(1)  # [B, M, r]

            # Step 3: project back to feature space
            delta_td = td_lowrank @ self.lora_B.T  # [B, M, C]

            # Step 4: residual — static TD + adaptive delta
            td = td + delta_td
    ...
    def forward(self, x, x_size, params, temb=None, x_lq=None):
        ...
        return self.norm(
            self.patch_embed(self.conv(
                self.patch_unembed(self.residual_group(x, x_size, params, temb, x_lq), x_size))) + x)
...
class MPv2_3_arch(nn.Module):
    """MPv2_3: LoRA-Conditioned Token Dictionary for Flow Matching SR.

    Key innovation over MPv2_2:
    - Static TD is decomposed into low-rank memory patterns (LoRA)
    - (lq_global + temb) generates per-pattern activation weights
    - Selectively extracts relevant memories per image/timestep
    - Zero-init ensures training starts equivalent to static ATD

    Forward: (x, c=None, temp=None) where x=6ch [x_t, lq_bicubic], split internally.
    """
    ...
    def forward_features(self, x, params, temb=None, x_lq=None):
        ...
        for layer in self.layers:
            x = layer(x, x_size, params, temb, x_lq)
        x = self.norm(x)
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # ===== 1. Split & Shallow Feature Extraction =====
        if self.upscale == 1:
            x_t = x
        ...
        # ===== 2. Deep Feature Extraction (LoRA-TD) =====
...
if __name__ == '__main__':
    model = MPv2_3_arch(
        ...
        temb_ch=256, embedding_dim=256, downscale=4, lora_rank=8,
    )
    ...
    # Test standard SR mode (upscale=4)
    model_sr = MPv2_3_arch(
        upscale=4, img_size=64, embed_dim=90,
        depths=[6, 6, 6, 6], num_heads=[6, 6, 6, 6], window_size=8,
        dim_ffn_td=16, category_size=128, num_tokens=64, reducted_dim=4,
        convffn_kernel_size=5, img_range=1., mlp_ratio=2, upsampler='pixelshuffle',
        lora_rank=8,
    )
    total_sr = sum(p.numel() for p in model_sr.parameters())
    print(f"SR Params: {total_sr / 1e6:.3f}M")
    x_sr = torch.randn(1, 3, 64, 64)
    out_sr = model_sr(x_sr)
    print(f"SR Input: {x_sr.shape} -> Output: {out_sr.shape}")
