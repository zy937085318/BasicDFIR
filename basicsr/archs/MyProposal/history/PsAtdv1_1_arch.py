...
# ==================== Timestep Embedding Modules (from PnPFlowUNet) ====================
...
def variance_scaling_init_(tensor, scale):
    ...
    var = gain / max(1., (fan_in + fan_out) / 2.)
...
class ATD_CA(nn.Module):#记忆模块使用的交叉注意力
...
class ATDTransformerLayer(nn.Module):
    ...
    def forward(self, x, td, x_size, params, temb=None):
        ...
        # ATD_CA  n, m, c是token数、记忆token数和通道数
        x_atd, sim_atd = self.attn_atd(x, td, x_size)  # x_atd: (b, n, c)  sim_atd: (b, n, m)  td: (b, m, c) sim_atd是每个特征与记忆中每个token的相似度，作为后续交叉注意力和分类注意力的依据

        # AC_MSA
        tk_id = torch.argmax(sim_atd, dim=-1, keepdim=False)#[b,n] 每个特征对应的最相似记忆token的id
        ...
        if temb is not None and self.temb_ch > 0:
            x = x + self.temb_proj(temb)[:, None, :]  # broadcast to all tokens
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, x_size, params, temb=None):
        b, n, c = x.shape
        td = self.td.expand([b, -1, -1])#复制b份记忆到batch维度
        idx_checkpoint = 5
...
class ATDB(nn.Module):
    ...
    def forward(self, x, x_size, params, temb=None):
        # return self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, temb), x_size))) + x
        return self.norm(self.patch_embed(self.conv(self.patch_unembed(self.residual_group(x, x_size, params, temb), x_size))) + x)
...
class PsATD1_1(nn.Module):
    def __init__(self,
        ...
        # ------------------------- Timestep Embedding for JiTModel ------------------------- #
        self.temb_ch = temb_ch
        self.embedding_dim = embedding_dim if embedding_dim is not None else embed_dim
        if temb_ch > 0:
        ...
        # Support both 3-channel (SR) and 6-channel (JiT) inputs
        if upscale > 1:
            # SR mode: 3 channels -> pixel_unshuffle
            self.pixel_unshuffle = nn.PixelUnshuffle(upscale)
            # Only use groups if embed_dim is divisible by upscale^2
            if embed_dim % (upscale ** 2) == 0:
                self.conv_first = nn.Conv2d(num_in_ch*upscale**2, embed_dim, 3, 1, 1, groups=upscale**2)
            else:
                self.conv_first = nn.Conv2d(num_in_ch*upscale**2, embed_dim, 3, 1, 1)
        else:
            # JiT mode: 6 channels direct input
            self.pixel_unshuffle = None
            # When in_chans=3, expand to 6 channels; when in_chans=6, use directly
            actual_in_ch = 6  # Always use 6 channels for JiT
            self.conv_first = nn.Conv2d(actual_in_ch, embed_dim, 3, 1, 1)
    ...
    def forward(self, x, c=None, temp=None):
        """
        JiTModel forward signature: (x, c=None, temp=None)
        x: [B, C, H, W] - C=3 for SR, C=6 for JiT (or C=3 will be expanded to 6)
        c: reference condition (currently unused, reserved for future)
        temp: timestep tensor [B] for flow matching
        """
        # ------------------------- Input Channel Expansion (3 -> 6) ------------------------- #
        # When upscale=1 (JiT mode), if input is 3 channels, expand to 6 channels
        if self.upscale == 1 and x.shape[1] == 3:
            x = torch.cat([x, x], dim=1)  # [B, 3, H, W] -> [B, 6, H, W]

        # ------------------------- Normalization ------------------------- #
        self.mean = self.mean.type_as(x)
        # For JiT mode (6 channels), skip RGB mean normalization
        if x.shape[1] == 3:
            x = (x - self.mean) * self.img_range
        else:
            # For JiT mode, only scale by img_range
            x = x * self.img_range

        # ------------------------- Pixel Unshuffle (SR mode only) ------------------------- #
        ...
        if temp is not None and self.temb_ch > 0:
            B = x.shape[0]
            if temp.dim() == 0:
                temp = temp.unsqueeze(0).expand(B)
            elif temp.shape[0] != B:
                temp = temp.expand(B)
            temb = self.temb_net(temp)
            assert list(temb.shape) == [B, self.temb_ch]
        ...
        # ------------------------- Denormalization ------------------------- #
        x = x / self.img_range
        # Add mean back only for 3-channel output (SR mode)
        if x.shape[1] == 3:
            x = x + self.mean
        ...
        if self.upscale > 1:
            ...
            # JiT mode: output size = input size
            x = x[..., :h_ori, :w_ori]
