'''
MPv2_2: Content-Conditioned Token Dictionary (CC-TD)
Core innovation: lq image guides Token Dictionary via FiLM modulation.

Design:
- x_t: main processing path (shallow conv → transformer → output)
- lq: ONLY used to modulate Token Dictionary (content-aware TD)
- t: simple additive timestep injection (proven sufficient)

Unlike MPv2 (timestep-conditioned TD) which failed because x_1 target is constant,
this conditions TD on lq content, providing direct image structure information.
Static TD (same for all images) → Content-conditioned TD (adapts to each image).
...
class ATD_CA(nn.Module):
    ...
    def forward(self, x, td, x_size):
        b, n, c = x.shape #[b,4096,90]
        b, m, c = td.shape #[b,64,90]
        q = self.wq(x)#[b,4096,4]
        k = self.wk(td)#[b,64,4]
        v = self.wv(td)
        attn = (F.normalize(q, dim=-1) @ F.normalize(k, dim=-1).transpose(-2, -1)) #[b,4096,64]
        attn = attn * (1 + torch.clamp(self.scale, 0, 3) * np.log(self.num_tokens)) #[b,4096,64]
        attn = self.softmax(attn) #[b,4096,64]
        return (attn @ v).reshape(b, n, c), attn#[b,4096,90]
...
class ATDTransformerLayer(nn.Module):
    """Standard ATD layer with simple additive timestep injection."""
...
class BasicBlock(nn.Module):
    """BasicBlock with Content-Conditioned Token Dictionary.

    Innovation: lq_global modulates TD via FiLM before each round of transformer layers.
    td_modulated = td * (1 + scale) + shift, where scale/shift come from lq content.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
                 ...
                 downsample=None, use_checkpoint=False, temb_ch=0):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.depth = depth
        ...
        # Token Dictionary (static base, modulated by lq at runtime)
        self.td = nn.Parameter(torch.randn([num_tokens, dim]), requires_grad=True)# [64, 90], lq[2, 4096, 90],是不是可以固定一个[64, 4096]的转移基于token，混合lr和记忆

        # ===== Content-Conditioned TD: FiLM modulation from lq =====
        self.lq_td_proj = nn.Sequential(
            nn.SiLU(),
            nn.Linear(dim, 2 * dim),  # scale + shift for TD
        )
        # Zero-init: at start, TD is unmodified (same as static ATD)
        nn.init.zeros_(self.lq_td_proj[-1].weight)
        nn.init.zeros_(self.lq_td_proj[-1].bias)

    def forward(self, x, x_size, params, temb=None, x_lq=None):
        b, n, c = x.shape
        td = self.td.expand([b, -1, -1])  # [B, M, C]
        idx_checkpoint = 5
...
class ATDB(nn.Module):
    def __init__(self, dim, idx, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = BasicBlock(
            ...
            norm_layer=norm_layer, downsample=downsample, use_checkpoint=use_checkpoint, temb_ch=temb_ch)
...
class MPv2_2_arch(nn.Module):
    """MPv2_2: Content-Conditioned Token Dictionary for Flow Matching SR.

    Key innovation:
    - lq (low-quality bicubic) is used ONLY to modulate Token Dictionary via FiLM
    - x_t is the main processing stream (shallow conv → transformer → output)
    - timestep injected via simple additive (proven sufficient for x_1 prediction)

    This addresses the finding that timestep conditioning on features doesn't help
    (target x_1 is constant), but lq-content conditioning on TD should help because:
    - TD stores image structure priors
    - Different images need different priors
    - lq provides the image content signal to select/adapt relevant priors
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3,
                 ...
                 resi_connection='1conv', temb_ch=0, embedding_dim=256, downscale=2, **kwargs):
        super().__init__()
        num_feat = 64
    ...
    def forward_features(self, x, params, temb=None, x_lq=None):
        ...
        x = self.patch_embed(x)#[2, 4096,90], 90是embedding size, 4096是patch number 64*64
        x_lq = self.lq_patch_embed(x_lq)#[2, 4096, 3]
        if self.ape:
    ...
    def forward(self, x, c=None, temp=None):
        ...
        if self.upscale == 1: #upscale=1 flowmatching模式, 否则 baseline ATD 模式
            x_t = x  # [B, 3, H, W] — x_t (noisy state)
            x_lq = c if c is not None else nn.functional.interpolate(x, scale_factor=1/self.downscale, mode='bilinear', align_corners=False)  # [b, 3, h, w]
            
            # Normalize
            x_lq = (x_lq - self.mean) * self.img_range

            # x_t main path
            x = self.conv_xt(self.pixel_unshuffle_xt(x_t)) #[B, embed_dim, h, w]
            x_lq = self.conv_lr(x_lq) #[B, embed_dim, h, w]
        ...
        # ===== 2. Deep Feature Extraction (CC-TD) =====
        #TODO 这里残差加上去的x是否应该用x_lq而不是x
