'''
MPv2_3g: Spectral Alpha Modulation (P3 iteration)

Based on MPv2_3b, adding learnable high-frequency energy modulation to alpha.

=== Improvement over MPv2_3b ===

Problem: In MPv2_3b, alpha is generated from raw x_lq with no frequency awareness.
The TD has no mechanism to adapt its pattern selection based on the frequency content
of the input or the current denoising timestep.

Solution: Keep the original alpha_proj from x_lq (preserving MPv2_3b behavior),
then modulate alpha with a learnable spectral gate derived from x_lq's high-frequency
energy. The spectral_mod layer is zero-initialized, so training starts identical
to MPv2_3b and gradually learns to modulate alpha based on frequency content.

Key changes from MPv2_3b:
- BasicBlock.__init__: add fixed HPF kernel + zero-init spectral_mod Linear(1, lora_rank)
- BasicBlock.forward: compute HF energy scalar -> spectral gate -> modulate alpha
- Zero-init guarantees safe ablation: starts as MPv2_3b, evolves from there
'''
...
@torch.no_grad()
def _make_laplacian_kernel(dim, kernel_size=3):
    """Create a fixed Laplacian high-pass filter for depthwise conv."""
    k = kernel_size
    kernel = torch.zeros(dim, 1, k, k)
    kernel[:, 0, :, :] = -1.0 / (k * k)
    center = k // 2
    kernel[:, 0, center, center] = (k * k - 1) / (k * k)
    return kernel


class BasicBlock(nn.Module):
    """BasicBlock with Spectral Alpha Modulation.

    Innovation: Keep original spatial-aware alpha from x_lq, then modulate it
    with a learnable spectral gate derived from x_lq's high-frequency energy.
    Zero-init ensures training starts identical to MPv2_3b.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
        ...
        # ===== Spatial-aware alpha projection (same as MPv2_3b) =====
        self.alpha_proj = nn.Sequential(
            nn.SiLU(),
            nn.Linear(dim, lora_rank, bias=False),
        )
        nn.init.zeros_(self.alpha_proj[-1].weight)

        # ===== Spectral modulation (NEW) =====
        # Maps scalar HF energy -> per-rank modulation gate
        # Zero-init: starts as no modulation (1+0=1), evolves during training
        self.spectral_mod = nn.Linear(1, lora_rank, bias=False)
        nn.init.zeros_(self.spectral_mod.weight)

        # Fixed Laplacian high-pass filter
        self.register_buffer('hpf_kernel', _make_laplacian_kernel(dim, kernel_size=3))

        self.temb_ch = temb_ch

    def forward(self, x, x_size, params, temb=None, x_lq=None):
        b, n, c = x.shape
        td = self.td.unsqueeze(0).expand(b, -1, -1)

        # ===== Spectral-modulated LoRA memory extraction =====
        if x_lq is not None:
            # 1. Original spatial-aware alpha (same as MPv2_3b)
            if temb is not None and self.temb_ch > 0:
                x_lq_t = x_lq + temb.unsqueeze(1)  # [B, N, C]
            else:
                x_lq_t = x_lq

            alpha_spatial = torch.sigmoid(self.alpha_proj(x_lq_t))  # [B, N, r]

            # 2. Spectral modulation from high-frequency energy
            h, w = x_size
            x_lq_2d = x_lq.transpose(1, 2).reshape(b, c, h, w).contiguous()
            x_lq_hf = F.conv2d(x_lq_2d, self.hpf_kernel, padding=1, groups=c)  # [B, C, H, W]
            hf_energy = x_lq_hf.pow(2).mean(dim=[1, 2, 3])  # [B] scalar energy

            # Learnable gate from energy (zero-init -> sigmoid(0) = 0.5, modulation = 1+0.5 = 1.5)
            # At init: spectral_mod weight=0 -> output=0 -> sigmoid(0)=0.5 -> gate=0.5
            # But we want gate=0 at init so modulation is identity. Use (2*sigmoid - 1) trick:
            spectral_gate = torch.sigmoid(self.spectral_mod(hf_energy.unsqueeze(-1)))  # [B, r]
            spectral_gate = 2.0 * spectral_gate - 1.0  # [B, r], range [-1, 1], init = 0

            # 3. Modulate alpha with spectral gate
            # alpha_spatial * (1 + gate), gate=0 at init -> no change
            alpha_spatial = alpha_spatial * (1 + spectral_gate.unsqueeze(1))  # [B, N, r]
            alpha = alpha_spatial.mean(dim=1)  # [B, r]

            # LoRA decomposition (same as MPv2_3b)
...
class MPv2_3g_arch(nn.Module):
    """MPv2_3g: Spectral Alpha Modulation + LoRA-TD.

    Key change from MPv2_3b: alpha is modulated by a learnable gate from x_lq's
    high-frequency energy. Zero-init ensures safe ablation starting point.
    """
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # Deep Feature Extraction (Spectral-Modulated LoRA-TD)
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 3, 256, 256)   # noisy HR (JiTv2 passes x separately from c)
    c = torch.randn(2, 3, 64, 64)     # LR reference
    temp = torch.rand(2)
    out = model(x, c, temp)
    print(f"Input: x={x.shape}, c={c.shape} -> Output: {out.shape}")
