'''
MPv2_3e: High-Frequency Conditioned LoRA Alpha (P3 iteration)

Based on MPv2_3b, replacing raw x_lq with high-frequency component for alpha generation.

=== Improvement over MPv2_3b ===

Problem: In MPv2_3b, alpha_proj takes raw x_lq features which contain both low-freq
structure and high-freq texture. The Token Dictionary then wastes capacity encoding
low-frequency patterns that are already well-represented by x_lq itself.

Solution: Apply a fixed Laplacian high-pass filter to x_lq before alpha generation.
The LoRA alpha is conditioned ONLY on high-frequency features, forcing the TD to
specialize in texture/detail pattern selection rather than redundant low-freq structure.

Key changes from MPv2_3b:
- BasicBlock.__init__: register fixed Laplacian HPF kernel as buffer (no learnable params)
- BasicBlock.forward: x_lq -> 2D -> HPF -> tokens -> alpha_proj (instead of raw x_lq)
- No changes to ATDTransformerLayer, ATDB, or top-level architecture
'''
...
def _make_laplacian_kernel(dim, kernel_size=3):
    ...
    Kernel sum = 0 (high-pass), center = (k^2-1)/k^2, surround = -1/k^2.
    """
    k = kernel_size
...
class BasicBlock(nn.Module):
    """BasicBlock with High-Frequency Conditioned LoRA Alpha.

    Innovation: Apply fixed Laplacian HPF to x_lq before alpha generation.
    TD's LoRA alpha is driven ONLY by high-frequency features, forcing TD
    to specialize in texture/detail pattern selection.
    """
    def __init__(self, dim, input_resolution, idx, depth, num_heads, window_size,
        ...
        # ===== High-frequency alpha projection =====
        # Input: x_lq HF tokens [B, N, dim] (temb added directly to tokens when temb_ch==dim)
        ...
        # Fixed Laplacian high-pass filter (not learnable)
    ...
    def forward(self, x, x_size, params, temb=None, x_lq=None):
        ...
        # ===== High-frequency conditioned LoRA memory extraction =====
        if x_lq is not None:
            # Convert x_lq tokens to 2D feature map for spatial filtering
            h, w = x_size
            x_lq_2d = x_lq.transpose(1, 2).reshape(b, c, h, w).contiguous()

            # Apply fixed Laplacian high-pass filter (depthwise)
            x_lq_hf = F.conv2d(x_lq_2d, self.hpf_kernel, padding=1, groups=c)

            # Convert back to tokens
            x_lq_hf_tokens = x_lq_hf.flatten(2).transpose(1, 2)  # [B, N, C]

            # Inject timestep into each HF token
            if temb is not None and self.temb_ch > 0:
                x_lq_hf_t = x_lq_hf_tokens + temb.unsqueeze(1)  # [B, N, C]
            else:
                x_lq_hf_t = x_lq_hf_tokens

            # Per-token alpha from high-frequency features, then spatial aggregation
            alpha_spatial = torch.sigmoid(self.alpha_proj(x_lq_hf_t))  # [B, N, r]
...
class MPv2_3e_arch(nn.Module):
    """MPv2_3e: High-Frequency Conditioned LoRA Alpha + LoRA-TD.

    Key change from MPv2_3b: alpha_proj takes HPF(x_lq) instead of raw x_lq,
    forcing TD to specialize in high-frequency texture patterns.
    """
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # Deep Feature Extraction (HF-Conditioned LoRA-TD)
