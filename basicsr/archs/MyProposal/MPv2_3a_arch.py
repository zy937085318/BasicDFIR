'''
MPv2_3a: AdaLN-Zero Timestep Conditioning (P2 ablation)

Based on MPv2_3_arch, replacing simple additive temb injection with DiT-style AdaLN-Zero.

=== Improvement over MPv2_3 ===

Problem (P2): In MPv2_3, temb is injected as simple additive `x += temb_proj(temb)`
after the attention block. This cannot modulate processing strategy based on noise level.

Solution: DiT-style 6-parameter AdaLN-Zero in each ATDTransformerLayer:
- shift1, scale1, gate1 for the attention branch (x_atd + attn_x + x_aca)
- shift2, scale2, gate2 for the ConvFFN branch
- Zero-init ensures the layer starts as identity (stable training)

Key difference from MPv2_3:
- ATDTransformerLayer: temb_proj replaced with adaLN_modulation
- norm1(x) is modulated before wqkv: modulate(norm1(x), shift1, scale1)
- attention output is gated: gate1 * (x_atd + attn_x + x_aca)
- norm2(x) is modulated before convffn: modulate(norm2(x), shift2, scale2)
- convffn output is gated: gate2 * convffn(...)

LoRA-TD mechanism remains unchanged from MPv2_3.
...
def modulate(x, shift, scale):
    """DiT-style modulation: x * (1 + scale) + shift."""
...
class ATDTransformerLayer(nn.Module):
    """ATD layer with DiT-style AdaLN-Zero timestep conditioning."""
    def __init__(self, dim, idx, input_resolution, num_heads, window_size, shift_size,
        ...
        # ===== AdaLN-Zero: 6-parameter modulation =====
        if temb_ch > 0:
            self.adaLN_modulation = nn.Sequential(
                nn.SiLU(),
                nn.Linear(temb_ch, 6 * dim, bias=True),
            )
    ...
    def forward(self, x, td, x_size, params, temb=None):
        ...
        # Modulated norm1 for attention input
        x_norm = self.norm1(x)
        if temb is not None and self.temb_ch > 0:
            x_norm = modulate(x_norm, shift1, scale1)
        ...
        attn_out = x_atd + attn_x.view(b, n, c) + x_aca
        if temb is not None and self.temb_ch > 0:
            x = shortcut + gate1.unsqueeze(1) * attn_out
        else:
            x = shortcut + attn_out

        # Modulated norm2 for FFN
        x_norm2 = self.norm2(x)
        if temb is not None and self.temb_ch > 0:
            x_norm2 = modulate(x_norm2, shift2, scale2)
            x = x + gate2.unsqueeze(1) * self.convffn(x_norm2, x_td, x_size)
        else:
            x = x + self.convffn(x_norm2, x_td, x_size)

        return x


class BasicBlock(nn.Module):
    """BasicBlock with LoRA-Conditioned Token Dictionary (same as MPv2_3)."""
...
class MPv2_3a_arch(nn.Module):
    """MPv2_3a: AdaLN-Zero timestep conditioning + LoRA-TD.

    Key change from MPv2_3: DiT-style 6-parameter AdaLN-Zero replaces simple additive temb.
    LoRA-TD and x+x_lq fusion unchanged.
    """
    ...
    def forward(self, x, c=None, temp=None):
        ...
        # Deep Feature Extraction (LoRA-TD + AdaLN-Zero)
