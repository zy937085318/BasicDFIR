...
# basicsr_arch_zoo copied from "A Variational Perspective on Diffusion-Based Generative Models and Score Matching", NeurIPS 2021, Huang et al
...
class Dual_TimestepEmbedding(nn.Module):
    def __init__(self, embedding_dim, hidden_dim, output_dim, act=Swish()):
        super().__init__()
        ...
        self.main = nn.Sequential(
            dense(2*embedding_dim, hidden_dim),
            act,
            dense(hidden_dim, output_dim),
        )
    ...
    def forward(self, t, r):
        ...
        remb = get_sinusoidal_positional_embedding(r, self.embedding_dim)
        temb = self.main(torch.cat([temb, remb], dim=1))
        return temb
