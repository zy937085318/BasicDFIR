        ...
        **ignored_kwargs,
        ...
        if scale_factor[0] == 4:
            stem_first = nn.Sequential(
                nn.Conv2d(in_chans, dims[0], kernel_size=4, padding=0, stride=4),
                LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
            )
        elif scale_factor[0] == 2:
            stem_first = nn.Sequential(
                nn.Conv2d(in_chans, dims[0], kernel_size=4, padding=1, stride=2),
                LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
            )
        elif scale_factor[0] == 1:
            stem_first = nn.Sequential(
                nn.ReflectionPad2d((1, 0, 1, 0)),
                nn.Conv2d(in_chans, dims[0], kernel_size=4, padding=0, stride=1),
                LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
            )
        else:
            raise NotImplementedError("only scale factors of 4, 2 and 1 supported for the first stem convolution")
        self.downsample_layers.append(stem_first)
        for i in range(3):
            if scale_factor[i+1] == 2:
                downsample_layer = nn.Sequential(
                    LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                    nn.Conv2d(dims[i], dims[i + 1], kernel_size=2, stride=2, padding=0),
                )
            elif scale_factor[i+1] == 1:
                downsample_layer = nn.Sequential(
                    LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                    nn.ReflectionPad2d((1, 0, 1, 0)),
                    nn.Conv2d(dims[i], dims[i + 1], kernel_size=2, stride=1, padding=0),
                )
            else:
                raise NotImplementedError("only scale factors of 2 and 1 supported for intermediate convolutional downsampling layers")
