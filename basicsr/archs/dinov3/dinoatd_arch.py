...
from basicsr.archs.dinounet_arch import ConvNeXtEncoder, dinov3


class Dino_Encoder(nn.Module):
    """
    DINOv3 Encoder with multi-stage feature extraction and configurable downsampling.
    Returns a list of features from all 4 stages with original DINO dimensions.

    Args:
        in_chans: Number of input channels (default: 3)
        dino_size: DINOv3 model size - 'tiny', 'small', 'base', or 'large'
        pretrained: Whether to load pretrained weights
        ds_scale: List of downsampling ratios for each stage [ds0, ds1, ds2, ds3]
                  ds0 can be 0, 2, or 4
                  ds1, ds2, ds3 can be 0 or 2
                  0 means no spatial downsampling (only channel change via 1x1 conv)
        freeze: Whether to freeze DINO backbone parameters except downsample layers.
                When True, freezes all stages and norm, keeping downsample_layers trainable.
    """
    def __init__(self, in_chans=3, dino_size='tiny', pretrained=True, ds_scale=None, freeze=False):
        super().__init__()
        # Import DINOv3 encoder
        from basicsr.archs.dinov3.convnext_encoder import (
            ConvNeXt_Encoder, convnext_sizes, LayerNorm
        )

        # Get config from dictionary (copy to avoid modifying original)
        dino_config = convnext_sizes[dino_size].copy()

        # Process ds_scale parameter
        if ds_scale is not None:
            downsample_ratios = []
            for i, ds in enumerate(ds_scale):
                if ds == 0:
                    downsample_ratios.append(1)  # stride=1, will be replaced with 1x1 conv
                else:
                    downsample_ratios.append(ds)
            dino_config['downsample_ratios'] = downsample_ratios

        # Create full 4-stage DINOv3 encoder
        self.dino = ConvNeXt_Encoder(
            in_chans=in_chans,
            depths=dino_config['depths'],
            dims=dino_config['dims'],
            downsample_ratios=dino_config['downsample_ratios'],
            pretrained_path=dino_config['pretrained_path'] if pretrained else ''
        )

        # Replace downsample layers with 1x1 conv where ds[i]=0
        if ds_scale is not None:
            self._replace_downsample_layers(ds_scale, dino_config['dims'], in_chans)

        # Fix first_conv padding when ds[0]=2 (use regular padding instead of reflection)
        if ds_scale is not None and ds_scale[0] == 2:
            self._fix_first_conv_padding(dino_config['dims'][0], in_chans)

        # Store dims for reference
        self.dims = dino_config['dims']  # e.g., [96, 192, 384, 768]
        self.ds_scale = ds_scale

        # Freeze DINO backbone except downsample layers if freeze=True
        if freeze:
            self._freeze_dino_backbone()

    def _replace_downsample_layers(self, ds_scale, dims, in_chans):
        """Replace downsample layers with 1x1 conv where ds[i]=0.

        When ds[i]=0, we want to keep spatial resolution but still change channels.
        """
        from basicsr.archs.dinov3.convnext_encoder import LayerNorm

        for i, ds in enumerate(ds_scale):
            if ds == 0:
                if i == 0:
                    # Stage 0: Replace first_conv with 1x1 conv (no spatial downsampling)
                    self.dino.downsample_layers[i] = nn.Sequential(
                        nn.Conv2d(in_chans, dims[i], kernel_size=1, stride=1, padding=0),
                        LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                    )
                else:
                    # Stage 1/2/3: Replace with LayerNorm + 1x1 conv (changes channels, no spatial downsampling)
                    self.dino.downsample_layers[i] = nn.Sequential(
                        LayerNorm(dims[i-1], eps=1e-6, data_format="channels_first"),
                        nn.Conv2d(dims[i-1], dims[i], kernel_size=1, stride=1, padding=0),
                    )
    def _fix_first_conv_padding(self, dim_0, in_chans):
        """Replace first_conv with regular padding when ds[0]=2.

        The original first_conv uses reflection padding. When ds[0]=2, we need
        to use regular padding=1 instead.
        """
        from basicsr.archs.dinov3.convnext_encoder import LayerNorm

        self.dino.downsample_layers[0] = nn.Sequential(
            nn.Conv2d(in_chans, dim_0, kernel_size=4, stride=2, padding=1),
            LayerNorm(dim_0, eps=1e-6, data_format="channels_first"),
        )

    def _freeze_dino_backbone(self):
        """Freeze all DINO parameters except downsample layers.

        When freeze=True, we freeze:
        - All stages (self.dino.stages)
        - Final norm layer (self.dino.norm)

        Downsample layers remain trainable for fine-tuning the resolution adaptation.
        """
        # Freeze all stages
        ...
        # Freeze final norm layer
        for param in self.dino.norm.parameters():
            param.requires_grad = False

        # downsample_layers remain trainable (default requires_grad=True)

    def forward(self, x):
        """
        Returns:
            feats: List of 4 feature maps from each stage
                  [feat0, feat1, feat2, feat3]
                  - feat0: [B, 96, H/4, W/4]    (after stage 0)
                  - feat1: [B, 192, H/4, W/4]   (after stage 1)
                  - feat2: [B, 384, H/4, W/4]   (after stage 2)
                  - feat3: [B, 768, H/4, W/4]   (after stage 3)
        """
        feats = []
        for i in range(4):
            x = self.dino.downsample_layers[i](x)
            x = self.dino.stages[i](x)
            feats.append(x)
        return feats
...
class DinoATD(nn.Module):
    def __init__(self,
        ...
        #     freeze=freeze
        # )
        self.conv_first = ConvNeXtEncoder(in_chans=num_in_ch, depths=dinov3[dino_size]['depths'],
                                          dims=dinov3[dino_size]['dims'], pretrained=dinov3[dino_size]['pretrained'],
                                          freeze_backbone=False, scale_factor=scale_factor)

        # self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)

        # Feature mapping layers: map each DINO stage output to embed_dim
        dino_dims = self.conv_first.dims  # e.g., [96, 192, 384, 768] for tiny
        self.feat_map = nn.ModuleList()
        for i, dim in enumerate(dino_dims):
            if i == len(dino_dims) - 1 and dim == embed_dim:
                # Last stage with matching dimension: skip mapping
                self.feat_map.append(nn.Identity())
            else:
                # Use 1x1 conv to map to embed_dim
                self.feat_map.append(nn.Conv2d(dim, embed_dim, kernel_size=1))
        ...
        self.patch_embed = PatchEmbed(
            img_size=img_size//self.upscale,
    ...
    def forward(self, x):
        ...
        # DINOv3 encoder returns list of 4 stage features
        dino_feats = self.conv_first(x)  # List of [feat0, feat1, feat2, feat3]
        # Map all features to embed_dim
        mapped_feats = []
        for i, feat in enumerate(dino_feats):
            mapped_feats.append(self.feat_map[i](feat))
        x_first = mapped_feats[-1]  # Use last mapped stage features: [B, embed_dim, H/4, W/4]
        # x_first = dino_feats
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            feat_h, feat_w = x_first.shape[-2], x_first.shape[-1]
            attn_mask = self.calculate_mask([feat_h, feat_w]).to(x.device)
            params = {'attn_mask': attn_mask, 'rpi_sa': self.relative_position_index_SA}
            # Note: No residual connection since x_first has different dims than original x
            # Need to add projection layer before forward_features
            x = self.conv_after_body(self.forward_features(x_first, params))
            ...
            attn_mask = self.calculate_mask([feat_h, feat_w]).to(x.device)
            params = {'attn_mask': attn_mask, 'rpi_sa': self.relative_position_index_SA}
            res = self.conv_after_body(self.forward_features(x_first, params))
