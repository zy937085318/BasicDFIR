...
class FeatureRefine(nn.Module):
    ...
    Processes ONLY the stage1 encoder feature before it enters the decoder.

    Design:
      - Takes encoder_features [stage0, stage1, stage2, stage3] as input
      - Refines only stage1 (H/8 resolution) with ATDB
      - Returns [stage0, refined_stage1, stage2, stage3]

    This reduces computation while still refining mid-level features.

    Definition follows ATD class pattern:
      - PatchEmbed/PatchUnEmbed for stage1 BCHW <-> BNC conversion
    ...
    def __init__(
        ...
        self.refine_idx = 1  # Stage1 index

        # Stage1 resolution (H/8, W/8 for scale_factor=4)
        refine_resolution = (img_size // (4 * (2 ** self.refine_idx)),
                              img_size // (4 * (2 ** self.refine_idx)))
        refine_dim = dims[self.refine_idx]

        # PatchEmbed / PatchUnEmbed / norm for stage1 only
        self.patch_embed = PatchEmbed(
            img_size=refine_resolution[0], patch_size=patch_size,
            in_chans=0, embed_dim=refine_dim)
        self.patch_unembed = PatchUnEmbed(
            img_size=refine_resolution[0], patch_size=patch_size,
            in_chans=0, embed_dim=refine_dim)
        self.norm = norm_layer(refine_dim)

        # Single ATDB block for stage1 refinement
        self.atdb = ATDB(
            dim=refine_dim,
            idx=self.refine_idx,
            input_resolution=refine_resolution,
            depth=depths[self.refine_idx],  # Use stage1 depth
            num_heads=num_heads[self.refine_idx],  # Use stage1 heads
    ...
    def forward(self, encoder_features):
        ...
        Returns:
            refined_features: [stage0, refined_stage1, stage2, stage3]
                             Only stage1 is refined by ATDB
        """
        # Build output list with stage1 replaced
        refined = [
            encoder_features[0],  # stage0: pass through
            None,                 # stage1: will be refined
            encoder_features[2],  # stage2: pass through
            encoder_features[3],  # stage3: pass through
        ]

        # Refine only stage1
        feat = encoder_features[self.refine_idx]
...
class Dino_1stATD_UNet(nn.Module):
    """
