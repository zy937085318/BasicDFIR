...
from basicsr.archs.unet_arch import downsample
...
class first_conv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size=4, stride=4, padding=0):
        super().__init__(in_channels, out_channels, kernel_size, stride, padding=0)
    def forward(self, input):
        # Calculate padding to make output size = input size / kernel_size
        h, w = input.shape[-2:]
        pad_h = self.kernel_size[-2] - self.stride[-2]
        pad_w = self.kernel_size[-2] - self.stride[-1]
...
class ConvNeXt_Encoder(nn.Module):
    ...
    def __init__(
        ...
        # scale_factor: int = 4,  # not used, only for compatibility with DINOv3's arch registry
        downsample_ratios=[4, 1, 1, 1],
        # DINO arguments
        ...
        **ignored_kwargs,
        ...
        stem_first = nn.Sequential(
            first_conv(in_chans, dims[0], kernel_size=4, stride=downsample_ratios[0]),
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
        )
        self.downsample_layers.append(stem_first)
        for i in range(3):
            downsample_layer = nn.Sequential(
                LayerNorm(dims[i], eps=1e-6, data_format="channels_first"),
                nn.ReflectionPad2d((1, 0, 1, 0)),
                nn.Conv2d(dims[i], dims[i + 1], kernel_size=2, stride=downsample_ratios[i + 1], padding=0),
            )
    ...
    def _get_intermediate_layers(self, x, n=1):
        h, w = x.shape[-2:]
        # print(h, w)
        # print(self.downsample_layers[0][0].weight.mean().item())
    ...
    def get_intermediate_layers(
        ...
        return outputs#[out[1] for out in outputs]#tuple(outputs)
...
convnext_sizes = {
    ...
    "small": dict(
        ...
        dims=[96, 192, 384, 768],
        downsample_ratios=[4, 1, 1, 1],
        pretrained_path='/8T2/pretrained_models/DINOV3_pretrained_weight/ConvNeXt/dinov3_convnext_small_pretrain_lvd1689m-296db49d.pth',
    ),
    "base": dict(
        depths=[3, 3, 27, 3],
        dims=[128, 256, 512, 1024],
        downsample_ratios=[4, 1, 1, 1],
        pretrained_path='/8T2/pretrained_models/DINOV3_pretrained_weight/ConvNeXt/dinov3_convnext_base_pretrain_lvd1689m-801f2ba9.pth',
    ),
    "large": dict(
        depths=[3, 3, 27, 3],
        dims=[192, 384, 768, 1536],
        downsample_ratios=[4, 1, 1, 1],
        pretrained_path='/8T2/pretrained_models/DINOV3_pretrained_weight/ConvNeXt/dinov3_convnext_large_pretrain_lvd1689m-61fa432d.pth',
    ),
}


def get_convnext_arch(arch_name, scale_factor=4):
    size_dict = None
    ...
    if scale_factor == 1:
        size_dict["downsample_ratios"] = [1, 1, 1, 1]
    elif scale_factor == 2:
        size_dict["downsample_ratios"] = [2, 1, 1, 1]
    elif scale_factor == 4:
        size_dict["downsample_ratios"] = [4, 1, 1, 1]
    elif scale_factor == 8:
        size_dict["downsample_ratios"] = [4, 2, 1, 1]
    elif scale_factor == 16:
        size_dict["downsample_ratios"] = [4, 2, 2, 1]
    elif scale_factor == 32:
        size_dict["downsample_ratios"] = [4, 2, 2, 2]
    else:
        raise NotImplementedError("unsupported scale factor:{}".format(scale_factor))
    return partial(
        ConvNeXt_Encoder,
        **size_dict,
    )
...
if __name__ == "__main__":
    model = get_convnext_arch("convnext_tiny", 32)().cuda()
