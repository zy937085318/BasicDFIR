# Adopted from https://github.com/andreas128/RePaint
...
def get_schedule_jump_test(to_supplement=False):
    ...
    ts = get_schedule_jump(t_T=t_T, n_sample=1, # 250
                           jump_length=jump_length, jump_n_sample=jump_n_sample,  # 10, 10
                           jump2_length=1, jump2_n_sample=1,
                           jump3_length=1, jump3_n_sample=1,
                           start_resampling=250)

    # # #
    ts = [((x - min(ts)) / (max(ts) - min(ts))) for x in ts]
    ts.reverse()
    # # #
    ...
    out_path = f"scheduler_visualization/schedule_{t_T}_{jump_length}_{jump_n_sample}.png"
    plt.savefig(out_path)
    print(out_path)
