...
from medical_data_augment_tool.transformations.intensity.np.normalize import scale_min_max, scale
from medical_data_augment_tool.utils.sitk_image import label_to_rgb, set_spacing_origin_direction
...
def create_image_layout_image(image, mode, data_format):
    ...
    if mode == 'max_projection' or mode == 'avg_projection' or mode == 'center_slice_projection':
        ...
        if mode == 'max_projection':
            projection_function = np_image.max_projection
        if mode == 'avg_projection':
            projection_function = np_image.avg_projection
        if mode == 'center_slice_projection':
            projection_function = np_image.center_slice_projection
...
def create_sitk_image(image, mode, data_format, spacing=None, origin=None, direction=None):
    ...
    if mode == 'split':
        ...
        return image_list
    if mode == 'gallery':
        return sitk.GetImageFromArray(np_image.gallery(list(np.rollaxis(image, axis=channel_axis))), isVector=False)
