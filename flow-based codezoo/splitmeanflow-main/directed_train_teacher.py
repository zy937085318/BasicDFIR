...
def main():
    ...
    # Create directories
    os.makedirs('./noised_images_DF2K', exist_ok=True)
    os.makedirs('./noised_results_DF2K', exist_ok=True)
    ...
    def sample_and_log_images():
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(val_dataloader):
                ...
                t = sampler.get_timestep_schedule(sample_steps=1)[-1]
                print(t)
                t = t.view(1,)
                samples = model(lq, t)
                # samples = sampler.sample_img_with_noise(lq, 10, return_all_steps=False)
            ...
            img_save_path = f"./noised_results_DF2K/step{step}.png"
    ...
    with tqdm(range(n_steps), dynamic_ncols=True) as pbar:
        ...
        for step in pbar:
            ...
            # print(x_t.shape)
            # print(t.shape)
            pred = model(lq, t, None)
            # print(model.final_layer.linear.weight.sum())

            # Compute losses
            positive_loss = F.mse_loss(pred, gt)
