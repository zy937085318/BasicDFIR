...
from pmf.dataset import get_loader
...
def train(config_path='./configs/pMFSR-B-16.yaml'):
    ...
    # Validation Dataset
    val_loader = get_loader(config, train=False)
    ...
    for epoch in range(config.num_epochs):
        ...
        for x, y in progress_bar:
            with accelerator.accumulate(model):
                # x is (N, C, H, W), y is (N,)
                loss, loss_dict = pmf_model.forward_loss(x, y)
        ...
        if accelerator.is_local_main_process:
            ...
            with torch.no_grad():
                for val_x, val_y in val_loader:
                    ...
                    loss, loss_dict = pmf_model.forward_loss(val_x, val_y)
            ...
            with torch.no_grad():
                # Sample some images
                z = torch.randn(4, config.in_channels, config.image_size, config.image_size, device=device)
                y_sample = torch.randint(0, config.num_classes-1, (4,), device=device)
                samples = pmf_model.sample(z, y_sample, cfg_scale=4.0)
