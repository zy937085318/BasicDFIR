...
from pnpflow.dit_transformer_2d import DiTTransformer2DModel
import warnings
import lpips
from basicsr.archs.jit_arch import JiT_B_16
...
def define_model(args):
    if args.model == "ot" or args.model == "gradient_step":
        model = UNet(input_channels=args.num_channels,
                     output_channels = args.num_channels//2,
        ...
        model_jit = JiT_B_16(
            ...
            out_channels = args.num_channels//2,
        )
        print('use jit model')
        return (model_jit, None)
...
def save_samples(samples, train_samples, path, args):
    ...
    cols = 1 #math.ceil(math.sqrt(batch_samples_size))  # Number of columns
    rows = math.ceil(batch_samples_size / cols)   # Number of rows
    fig, ax = plt.subplots(rows, 2 * cols, figsize=(10, 20))
    for i in range(rows):
        for j in range(cols):
